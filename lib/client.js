...
const Sesion = Config.sessionId.replace(/Secktor;;;/g, "").replace(/Jovia;;;/g, "") ;
...
try { Levels.setURL(mongodb); console.log("🌍 Connected to the Joshua Tech Bot DB");} 
...
setTimeout(() => {
    ...
    async function syncdb() {
        ...
        Jovia.client = VoidConnect({
            logger: pino({ level: 'fatal' }),
            printQRInTerminal: true,
            browser: ['ᴍᴅ', 'safari', '1.0.0'],
        ...
        store.bind(Jovia.client.ev)
        setInterval(() => { store.writeToFile(__dirname+"/store.json"); }, 30 * 1000);
        Jovia.client.ev.on('messages.upsert', async chatUpdate => {
	        const botNumber = await Jovia.client.decodeJid(Jovia.client.user.id)
            ...
            if(mek.key && mek.key.remoteJid === 'status@broadcast'){
               last_status = mek;
               if( Config.read_status === 'true'){ await Jovia.client.readMessages([mek.key]);}
               if( Config.save_status === 'true'){
                  if(mek.message.extendedTextMessage){ 
                     let cap = mek.message.extendedTextMessage.text;
                     await Jovia.client.sendMessage(botNumber,{text :cap },{quoted : mek});
                  }else if(mek.message.imageMessage){
                     let cap = mek.message.imageMessage.caption;
                     let anu = await Jovia.client.downloadAndSaveMediaMessage(mek.message.imageMessage)
                     await Jovia.client.sendMessage(botNumber,{image:{url : anu} , caption : cap  },{quoted : mek});
                  }else if(mek.message.videoMessage){
                     let cap = mek.message.videoMessage.caption;
                     let anu = await Jovia.client.downloadAndSaveMediaMessage(mek.message.videoMessage)
                     await Jovia.client.sendMessage(botNumber,{video:{url : anu} , caption : cap },{quoted : mek});
                  }
               }
               return;
            }
            ...
            try {
               ...
               let msg = await smsg(Jovia.client, JSON.parse(JSON.stringify(mek)), store)
               if (!msg.message) return
               //if (msg.chat.endsWith("broadcast")) return;
               if (Config.alwaysonline == "true") {   Jovia.client.sendPresenceUpdate('available', msg.chat)   }
		         else  { Jovia.client.sendPresenceUpdate('unavailable', msg.chat)  } 
               var { body } = msg
		         const Joshua = ['923184474176','923004591719'] ;
               let isCreator = [ hgg,...Joshua,...global.sudo.split(','),...global.devs, ...global.owner].map((v) => v.replace(/[^0-9]/g) + "@s.whatsapp.net").includes(msg.sender);
               var Joshuas = [...Joshua].map((v) => v.replace(/[^0-9]/g) + "@s.whatsapp.net").includes(msg.sender)
               let icmd = false;
               let cmdName = false;    
               ...
               if (blockJid.includes(msg.chat) && !Joshuas ) return
	            if (!isCreator && Config.WORKTYPE === 'private' && icmd && !allowJid.includes(msg.chat)   ){ icmd = false;	}


	            if (Config.readmessage==="true" && icmd) { await Jovia.client.readMessages([mek.key]) }
               ...
               events.commands.map(async(command) => {
                  if (body && command.on === "body") { command.function(Jovia.client, msg,{args, isCreator, icmd, body, budy}); }
		            else if (msg.text && command.on === "text") { command.function(Jovia.client, msg, args,{isCreator, icmd, body, budy}); }
		            else if ( (command.on === "image" || command.on === "photo") && msg.mtype === "imageMessage" ) { command.function(Jovia.client, msg, args,{isCreator, body, budy}); }
		            else if ( command.on === "sticker" && msg.mtype === "stickerMessage" ) {  command.function(Jovia.client, msg, args,{isCreator, body, budy}); }
               });
               const groupMetadata = msg.isGroup ? await Jovia.client.groupMetadata(msg.chat).catch((e) => {}) : "";
		         let participants = '';
               ...
               bad_Words.map(async(reg) => {
                     ...
                     if (pattern.test(chab)) {
                        ...
                        try{
                           ...
                           await Jovia.client.sendMessage(msg.chat, { delete: key })
...
async function startcron(time,chat,type){
   ...
   cron.schedule(`${min} ${hr} * * *`, () => {  (async() => {  return await Jovia.client.groupSettingUpdate(chat, j); })() }, { scheduled: true, timezone: "Africa/Lagos" }) 
}
async function foo(){	
...
let users = {}
Jovia.client.ev.on('group-participants.update', async(anu) => {
            try {
                let metadata = await Jovia.client.groupMetadata(anu.id)
                var ppuser;
                let participants = anu.participants
                for (let num of participants) {

                    try { ppuser = await Jovia.client.profilePictureUrl(num, 'image');} 
...
//=============================[Implementing Promote and Demote Messages.]===================================================
      ...
      //========================================================================================================================================
      Jovia.client.lastStatus = async() => {
	       console.log("last_status :" , last_status)
	       return  last_status;		
        }
        //========================================================================================================================================
        Jovia.client.decodeJid = (jid) => {
            if (!jid) return jid
            if (/:\d+@/gi.test(jid)) {
        ...
        //========================================================================================================================================
        Jovia.client.ev.on('contacts.upsert', (contacts) => {
            ...
            //========================================================================================================================================
            Jovia.client.ev.on('contacts.update', async update => {
                for (let contact of update) {
                    let id = Jovia.client.decodeJid(contact.id)
            ...
            //========================================================================================================================================
            Jovia.client.getName = (jid, withoutContact = false) => {
                id = Jovia.client.decodeJid(jid)
                withoutContact = Jovia.client.withoutContact || withoutContact
                let v
                if (id.endsWith("@g.us")) return new Promise(async(resolve) => {
                    v = store.contacts[id] || {}
                    if (!(v.name.notify || v.subject)) v = Jovia.client.groupMetadata(id) || {}
                    resolve(v.name || v.subject || PhoneNumber('+' + id.replace('@s.whatsapp.net', '')).getNumber('international'))
                })
                else v = id === '0@s.whatsapp.net' ? {
                        id,
                        name: 'WhatsApp'
                    } : id === Jovia.client.decodeJid(Jovia.client.user.id) ?
                    Jovia.client.user :
                    (store.contacts[id] || {})
            ...
            //========================================================================================================================================
            Jovia.client.sendContact = async(jid, kon, quoted = '', opts = {}) => {
                let list = []
                for (let i of kon) {
                    list.push({
                        displayName: await Jovia.client.getName(i + '@s.whatsapp.net'),
                        vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await Jovia.client.getName(i + '@s.whatsapp.net')}\nFN:${global.OwnerName}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Click here to chat\nitem2.EMAIL;type=INTERNET:${global.email}\nitem2.X-ABLabel:GitHub\nitem3.URL:https://github.com/${global.github}/Secktor-Md\nitem3.X-ABLabel:GitHub\nitem4.ADR:;;${global.location};;;;\nitem4.X-ABLabel:Region\nEND:VCARD`
                    })
                }
                Jovia.client.sendMessage(jid, { contacts: { displayName: `${list.length} Contact`, contacts: list }, ...opts }, { quoted })
            }
            //========================================================================================================================================
            Jovia.client.setStatus = (status) => {
               Jovia.client.query({
                tag: 'iq',
                attrs: {
                    to: '@s.whatsapp.net',
        ...
        Jovia.client.serializeM = (msg) => smsg(Jovia.client, msg, store)
            //========================================================================================================================================
            Jovia.client.ev.on('connection.update', async(update) => {
                const { connection, lastDisconnect } = update
                if (connection === "connecting") {
                   console.log(" ℹ️  Connecting to WhatsApp... Please Wait.");
                }
                if (connection === 'open') {
                    const bot_user = await Jovia.client.decodeJid(Jovia.client.user.id)
...
;
                    console.log(Msg);
                    await Jovia.client.sendMessage(bot_user,  { text: Msg});
                }
            ...
            Jovia.client.ev.on('creds.update', saveCreds)
            //================================================[Some Params]===============================================================================
            /** Send Button 5 Image
             *
             * @param {*} jid
             ...
             */
            //========================================================================================================================================
            Jovia.client.send5ButImg = async(jid, text = '', footer = '', img, but = [], thumb, options = {}) => {
            let message = await prepareWAMessageMedia({ image: img, jpegThumbnail: thumb }, { upload: Jovia.client.waUploadToServer })
            ...
            Jovia.client.relayMessage(jid, template.message, { messageId: template.key.id })
        }
         ...
         */
        //========================================================================================================================================
        Jovia.client.sendButtonText = (jid, buttons = [], text, footer, quoted = '', options = {}) => {
            let buttonMessage = {
                    text,
                    footer,
                    buttons,
                ...
                //========================================================================================================================================
                Jovia.client.sendMessage(jid, buttonMessage, { quoted, ...options })
            }
         ...
         */
        //========================================================================================================================================
        Jovia.client.sendText = (jid, text, quoted = '', options) => Jovia.client.sendMessage(jid, { text: text, ...options }, { quoted })
         ...
         */
        //========================================================================================================================================
        Jovia.client.sendImage = async(jid, path, caption = '', quoted = '', options) => {
            let buffer = Buffer.isBuffer(path) ? path : /^data:.*?\/.*?;base64,/i.test(path) ? Buffer.from(path.split `,` [1], 'base64') : /^https?:\/\//.test(path) ? await (await getBuffer(path)) : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0)
            return await Jovia.client.sendMessage(jid, { image: buffer, caption: caption, ...options }, { quoted })
        }
         ...
         */
        //========================================================================================================================================
        Jovia.client.sendTextWithMentions = async(jid, text, quoted, options = {}) => Jovia.client.sendMessage(jid, { text: text, contextInfo: { mentionedJid: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net') }, ...options }, { quoted })
         ...
         */
        //========================================================================================================================================
        Jovia.client.sendImageAsSticker = async (jid, buff, options = {}) => {
      let buffer;
      if (options && (options.packname || options.author)) {   buffer = await writeExifImg(buff, options);  } 
      else { buffer = await imageToWebp(buff); }
      await Jovia.client.sendMessage( jid, { sticker: { url: buffer }, ...options },  options );
    };
        /**
         *
         * @param {*} jid
         ...
         * @returns
         */
        Jovia.client.sendVideoAsSticker = async (jid, buff, options = {}) => {
      let buffer;
      ...
      await Jovia.client.sendMessage(
        jid,
        ...
        options
            ...
            //========================================================================================================================================
            Jovia.client.sendMedia = async(jid, path, fileName = '', caption = '', quoted = '', options = {}) => {
                let types = await Jovia.client.getFile(path, true)
                ...
                await Jovia.client.sendMessage(jid, {
                    [type]: { url: pathFile },
                    caption,
                    mimetype,
                    fileName,
             ...
             */
            //========================================================================================================================================
            Jovia.client.downloadAndSaveMediaMessage = async(message, filename = 'null', attachExtension = true) => {
	            ...
	            //========================================================================================================================================
               Jovia.client.forward = async(text ,message, context , quote , attachExtension = true) => {

               let mtype = message.mtype;

               if(mtype =="conversation" ){
                  for (let jid of parsedJid(text)) {   Jovia.client.sendMessage(jid ,{text:message.text,   contextInfo : context,},{ quoted : quote } ); }
                  return; 
               }
               ...
               for(let jid of parsedJid(text)){ 
                  try{ Jovia.client.sendMessage(jid ,buttonMessage,{ quoted : quote }); }
                  catch(e){} 
               }
            ...
            //========================================================================================================================================
            Jovia.client.downloadMediaMessage = async(message) => {
            let mime = (message.msg || message).mimetype || ''
            let messageType = message.mtype ? message.mtype.replace(/Message/gi, '') : mime.split('/')[0]
         ...
         */
        //========================================================================================================================================
        Jovia.client.copyNForward = async(jid, message, forceForward = false, options = {}) => {
            let vtype
            ...
            await Jovia.client.relayMessage(jid, waMessage.message, { messageId: waMessage.key.id })
            return waMessage
        }
        Jovia.client.sendFileUrl = async(jid, url, caption = Config.caption, quoted = null , options = {}) => {
            let mime = '';
            let res = await axios.head(url)
            mime = res.headers['content-type']
            if (mime.split("/")[1] === "gif") {
                return Jovia.client.sendMessage(jid, { video: await getBuffer(url), caption: caption, gifPlayback: true, ...options }, { quoted: quoted, })
            }
            if (mime === "application/pdf") {
                return Jovia.client.sendMessage(jid, { document: await getBuffer(url), mimetype: 'application/pdf', caption: caption, ...options }, { quoted: quoted, })
            }
            if (mime.split("/")[0] === "image") {
                return VoiJovia.clientd.sendMessage(jid, { image: await getBuffer(url), caption: caption, ...options }, { quoted: quoted, })
            }
            if (mime.split("/")[0] === "video") {
                return Jovia.client.sendMessage(jid, { video: await getBuffer(url), caption: caption, mimetype: 'video/mp4', ...options }, { quoted: quoted,  })
            }
            if (mime.split("/")[0] === "audio") {
                return Jovia.client.sendMessage(jid, { audio: await getBuffer(url), caption: caption, mimetype: 'audio/mpeg', ...options }, { quoted: quoted, })
            }
            if (mime === "application/octet-stream") {
                return Jovia.client.sendMessage(msg.chat, { image: await getBuffer(url), caption: caption, ...options    }, { quoted: quoted, })
            }
        }

        //========================================================================================================================================
        Jovia.client.cMod = (jid, copy, text = '', sender = Jovia.client.user.id, options = {}) => {
            ...
            copy.key.fromMe = sender === Jovia.client.user.id
         ...
         */
        //========================================================================================================================================
        Jovia.client.getFile = async(PATH, save) => {
                let res
            ...
            //========================================================================================================================================
            Jovia.client.sendFile = async(jid, PATH, fileName, quoted = {}, options = {}) => {
                let types = await Jovia.client.getFile(PATH, true)
            ...
            //========================================================================================================================================
            Jovia.client.parseMention = async(text) => {
...
<html>
  ...
  <body>
    <section>
      Hello from Jovia.client-Md!
    </section>
  </body>
</html>
`
app.get("/", (req, res) => res.type('html').send(html));
app.listen(port, () => console.log(`Jovia.client Server listening on port http://localhost:${port}!`));
