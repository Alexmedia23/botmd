...
//-----------------------------------------------------------------------
amd({pattern: "waifu",desc: "To get Waifu Random Pics",category: "Anime Pics",filename: __filename },async(Jovia, msg, text , {cmdName}) => { return await sendAnime(Jovia,msg ,cmdName , text);});
//-----------------------------------------------------------------------
amd({pattern: "neko", category: "Anime Pics",desc: "Sends a Neko Image in chat",filename: __filename },async(Jovia, msg, text , {cmdName}) => { return await sendAnime(Jovia,msg ,cmdName , text);});
//-----------------------------------------------------------------------
amd({ pattern: "megumin",desc: "To get Waifu Random Pics",category: "Anime Pics",filename: __filename },async(Jovia, msg, text , {cmdName}) => { return await sendAnime(Jovia,msg ,cmdName , text);});
//-----------------------------------------------------------------------
amd({ pattern: "loli",category: "Anime Pics",filename: __filename,desc: "Sends image of loli."},async(Jovia, msg) => {  return await sendAnime(Jovia,msg ,'loli',);});
//-----------------------------------------------------------------------
amd({pattern: "foxgirl",category: "Anime Pics",desc: "Sends image of Fox Girl Anime.", filename: __filename},async(Jovia, msg) => {  return await sendAnime(Jovia,msg ,'loli',); });
//-----------------------------------------------------------------------
amd({pattern: "demon",alias : ["ds"],desc: "To get Naruto Random Videos",category: "Anime Pics",filename: __filename},async(Jovia, msg) => {  return await sendAnime(Jovia,msg ,'demon',); });
//------------------------------------------------------------------------
amd({pattern: "naruto",desc: "To get Naruto Random Videos",category: "Anime Pics",filename: __filename},async(Jovia, msg) => {  return await sendAnime(Jovia,msg ,'naruto',); });
//-------------------------------------------------------------------------
amd({pattern: "pokepic",category: "Anime Pics",filename: __filename,desc: "Sends image of pokemon."},async(Jovia, msg, text) => { return await sendGImages(msg ,text+"Pokemon Pics only HD ", `*---「 Poke Pic 」---*`, text );});
//-------------------------------------------------------------------------
amd({pattern: "animepic",category: "Anime Pics",filename: __filename,desc: "Anime images"},async(Jovia, msg, text) => { return await sendGImages(msg ,text+"Anime Pics HD", `*-----「 Anime Image 」-----*`, text );});
//-----------------------------------------------------------------------
amd({ pattern: "animewall",category: "Anime Pics",desc: "Anime Wallpaper Random",filename: __filename},async(Jovia, msg, text) => {return await sendGImages(msg ,text+"anime wallpaper for desktop full hd", `*---「 Anime Wallpaper 」---*` , text );});
//-----------------------------------------------------------------------
...
amd({ pattern: "animenews",category: "Anime Pics", desc: "Sends Anime News in chat",filename: __filename},async(Jovia,msg,text,{cmdName}) => {let q1=qq[Math.floor(Math.random()*qq.length)]+text;return await sendAnime(Jovia,msg,cmdName, q1);});
//---------------------------------------------------------------------------
amd({pattern: "pokemon", category: "Anime Pics",filename: __filename, desc: "Sends info of pokemon in current chat."},async(Jovia, msg, text) => { if(!text) return msg.send("*Uhh Please Give Me Poki Name/num*");try { let { data: data } = await axios.get(`https://pokeapi.co/api/v2/pokemon/${text}`);if (!data.name) return msg.reply(`❌ Could not found any pokemon with that name`); let poinfo = `*•Name: ${data.name}*\n*•Pokedex ID: ${data.id}*\n*•Height: ${data.height}*\n*•Weight: ${data.weight}*\n*•Abilities: ${data.abilities[0].ability.name}, ${data.abilities[1].ability.name}*\n*•Base Experience: ${data.base_experience}*\n*•Type: ${data.types[0].type.name}*\n*•Base Stat: ${data.stats[0].base_stat}*\n*•Attack: ${data.stats[1].base_stat}*\n*•Defense: ${data.stats[2].base_stat}*\n*•Special Attack: ${data.stats[3].base_stat}*\n*•Special Defense:${data.stats[4].base_stat}*\n*•Speed: ${data.stats[5].base_stat}*\n`; return await Jovia.client.sendMessage(msg.chat, { image: { url: data.sprites.front_default }, caption: poinfo }, { quoted: msg }); } catch (err) {  msg.reply("Ahh,Couldn't found any pokemon.") }})
//---------------------------------------------------------------------------
...
async(Jovia, msg, text) => {
    ...
    Jovia.client.sendMessage(msg.chat, {  image: {  url: srh.data[0].images.jpg.large_image_url,  }, caption: mang, }, {  quoted: msg,  });
...
async(Jovia, msg, text) => {
 ...
 Jovia.client.sendMessage( msg.chat, { image: {  url: result.images.jpg.large_image_url, }, caption: details, }, { quoted: msg,});
}
   )
//---------------------------------------------------------------------------
